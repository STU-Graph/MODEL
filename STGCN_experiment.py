...
class STConvBlock(nn.Module):
    ...
    def forward(self, x):  # x: (batch_size, feature_dim/c[0], input_length, num_nodes)
        ...
        x=x_t1.permute(0,2,3,1)
        r1 = np.loadtxt('./distance08.csv', delimiter=',', usecols=0, skiprows=1)
        r2 = np.loadtxt('./distance08.csv', delimiter=',', usecols=1, skiprows=1)
        edge_index = torch.tensor([r1, r2], dtype=torch.long).cuda()
        c=[]
        for i in x:
            b=[]
            for i1 in i:
                x11 = torch.tensor(i1, dtype=torch.float).cuda()
                x111=Data(x=x11,edge_index=edge_index)
                xx=self.GAT(x111.x,x111.edge_index)
                b.append(xx)
            b_tensor=torch.stack(b)
            c.append(b_tensor)
        c_tensor=torch.stack(c)
        x_s=c_tensor.permute(0,3,1,2)
